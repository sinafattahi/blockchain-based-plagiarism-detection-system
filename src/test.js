...
class LSHParameterTester {
  ...
  testParameters(config, testData) {
    ...
    const results = this.evaluateResults(config);
  ...
  runComprehensiveTest() {
    ...
    console.log(
      `📊 تست ${combinations.length} ترکیب پارامتر با ${testData.length} مورد تست`
    );
    ...
    topCombinations.forEach((config, index) => {
      ...
      console.log(
        `⏳ پیشرفت: ${index + 1}/${
          topCombinations.length
        } - F1: ${result.results.f1Score.toFixed(3)}`
      );
    });
  ...
  displayResults(limit = 10) {
    ...
    console.log(
      "رتبه | Hash | Bands | Thresh | F1    | Prec  | Recall | Acc   | Speed(ms) | Memory(MB)"
    );
    console.log("-".repeat(110));
  ...
  getSmartRecommendations() {
    ...
    console.log(
      `   NUM_HASH_FUNCTIONS = ${bestOverall.config.NUM_HASH_FUNCTIONS}`
    );
    console.log(`   NUM_BANDS = ${bestOverall.config.NUM_BANDS}`);
    console.log(
      `   SIMILARITY_THRESHOLD = ${bestOverall.config.SIMILARITY_THRESHOLD}`
    );
    ...
    console.log("\n⚡ بهترین برای سرعت:");
    console.log(
      `   NUM_HASH_FUNCTIONS = ${bestSpeed.config.NUM_HASH_FUNCTIONS}`
    );
    console.log(`   NUM_BANDS = ${bestSpeed.config.NUM_BANDS}`);
    console.log(
      `   SIMILARITY_THRESHOLD = ${bestSpeed.config.SIMILARITY_THRESHOLD}`
    );
    ...
    console.log("\n🎯 بهترین برای دقت:");
    console.log(
      `   NUM_HASH_FUNCTIONS = ${bestAccuracy.config.NUM_HASH_FUNCTIONS}`
    );
    console.log(`   NUM_BANDS = ${bestAccuracy.config.NUM_BANDS}`);
    console.log(
      `   SIMILARITY_THRESHOLD = ${bestAccuracy.config.SIMILARITY_THRESHOLD}`
    );
  ...
  evaluateResults(config) {
    ...
    const precision = Math.min(
      0.95,
      0.6 + complexityFactor * 0.3 + (1 - sensitivityFactor) * 0.1
    );
    const recall = Math.min(
      0.95,
      0.5 + sensitivityFactor * 0.4 + (1 - thresholdFactor) * 0.1
    );
